const CreateVenuePage = {
  ...
  initializeMap: async function () {
    try {
      console.log("Initializing venue location map");
      ...
      this.venueLocationMap = await MapService.initializeVenueMap(
        "venue-location-map",
        {
  ...
  useCurrentLocation: async function () {
    ...
    try {
      ...
      const locationInfo = await MapService.reverseGeocode(
        location.lng,
        location.lat,
      );
  ...
  toggleLocationPicker: function () {
    ...
    if (this.isLocationPickerEnabled) {
      ...
      Utils.showInfo(
        "Location picker disabled. Click the button to enable it again.",
      );
    } else {
      // Enable picker
  ...
  handleFormSubmission: async function () {
    try {
      ...
      if (imageUrl) {
        ...
        console.log("No image uploaded, creating venue without image");
      }
  ...
  gatherFormData: function () {
    const formData = {
      ...
      capacity: document.getElementById("venueCapacity").value
        ? parseInt(document.getElementById("venueCapacity").value)
        : null,
    ...
    Object.keys(formData).forEach((key) => {
      if (formData[key] === "" || formData[key] === null) {
        ...
        if (
          !["name", "type", "address", "latitude", "longitude"].includes(key)
        ) {
          delete formData[key];
        }
      }
  ...
  removeImagePreview: function () {
    ...
    if (previewContainer) {
      previewContainer.style.display = "none";
            ...
            console.log("Image preview removed and uploaded URL cleared");
  },
